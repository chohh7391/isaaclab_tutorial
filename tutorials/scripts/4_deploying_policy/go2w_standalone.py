...
from go2w import Go2WFlatTerrainPolicy
...
# spawn world
my_world = World(stage_units_in_meters=1.0, physics_dt=0.005, rendering_dt=0.01)
...
for i in range(0, args.num_robots):
    go2w = Go2WFlatTerrainPolicy(
        prim_path="/World/Go2W_" + str(i),
        name="Go2W_" + str(i),
        usd_path=f"{USD_DIR}/go2w_description/go2w_description.usd",
        position=np.array([0, i, 0.45]),
    )
